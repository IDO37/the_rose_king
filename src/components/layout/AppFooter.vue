...
const navigation = {
  ...
  social: [
    {
      name: 'GitHub',
      href: 'https://github.com/yourusername/rosenkoenig',
      icon: () => (
    ...
    <div class="mx-auto max-w-7xl overflow-hidden py-12 px-4 sm:px-6 lg:px-8">
      <nav class="-mx-5 -my-2 flex flex-wrap justify-center" aria-label="Footer">
        <div v-for="item in navigation.main" :key="item.name" class="px-5 py-2">
          <router-link
            :to="item.href"
            class="text-base text-gray-500 hover:text-rose-600 dark:text-gray-400 dark:hover:text-rose-400"
          >
            {{ item.name }}
          </router-link>
        </div>
      ...
      <div class="mt-8 flex justify-center space-x-6">
        <a
          ...
          class="text-gray-400 hover:text-rose-600 dark:hover:text-rose-400"
        >
      ...
      <p class="mt-8 text-center text-base text-gray-400">
        &copy; {{ currentYear }} Rosenkönig. All rights reserved.
      </p>
      <p class="mt-2 text-center text-xs text-gray-400">
        v{{ $config.app.version }} - {{ $config.app.mode }}
      </p>
    </div>
  </footer>
</template>
